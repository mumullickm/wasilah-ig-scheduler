"""Wasilah blog-link auto-publisher: Facebook Page link posts.
...
Posts anything due-but-unposted, so a late or skipped cron self-heals on the
next tick. Secret: META_PAGE_TOKEN.
"""
...
def main():
    ...
    due = [s for s in sched
           ...
           and not state.get(s["slug"])]
    if not due:
        print(f"[{stamp}] no links due")
        return

    for s in due:
        ok, info = publish_fb_link(page_id, s)
        state[s["slug"]] = info if ok else False
        print(f"[{stamp}] FB link {s['slug']}: {'published ' + info if ok else info}")
